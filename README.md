<!--
  Profile README for github.com/FarhanDipto
  Custom images live in assets/ and are generated by assets/build_svgs.py.
  Everything else is a live badge or card from a public service.
  Setup notes for the 3D calendar are in SETUP.md.
-->
...
Before that, I was doing federated learning and edge-fog-cloud systems research at **AISIP Lab**, with a detour building OTT streaming backends. CSE graduate of **Ahsanullah University of Science & Technology**.

I mostly write systems that turn a sentence into something real: a slide deck, a spreadsheet, a retrieved answer. Then I spend an unreasonable amount of time making sure the output doesn't quietly lie to the renderer.
...
A spec-driven PowerPoint engine. An LLM emits a content-only `DeckSpec`, and everything about how the deck *looks* (palette, typography, composition, icons) is decided downstream by code, not the model.
...
<details>
<summary><b>⚙️ A few things worth knowing about deckforge</b></summary>
<br/>

- Text is measured against real TTF outlines, not guessed. `fit_size` binary-searches the largest point size that fits a box, so a three-word headline and a full paragraph share the same box with no hand-tuned constants.
- Contrast repair walks *lightness* while holding *hue*. A brand green under `ensure_contrast` stays green instead of washing out to grey.
- Distinguishability and legibility are treated as separate problems: a Delta-E check catches near-identical hues (like red vs. green at 1.47:1 luminance) that a plain WCAG contrast check would wave through.
- 18 runtime modules compile into one 6,389-line production file. The test suite requires two structurally different design systems to share **less than 35%** of their text geometry on identical content. Current result across every pair: **0%**.

</details>

<details>
<summary><b>📐 A few things worth knowing about xlsx_gen</b></summary>
<br/>

- The model never writes a formula. It describes intent (`sum_row`, `growth_prev`, `ref_sum` across sheets) and a deterministic renderer resolves real cell coordinates into the actual Excel formula.
- Cross-sheet references are two-pass: render every block first and record where it landed in a registry, then resolve every pending reference against it.
- A single malformed block is skipped with a logged traceback instead of taking down the whole workbook.
- `preflight.scan()` exists because Excel silently "repairs" bad files by deleting fills and number formats. LibreOffice and Google Sheets, meanwhile, render the exact same broken file just fine, so nothing but an Excel-aware check catches it.
...
<!-- Generated by .github/workflows/profile-3d-contrib.yml in the palette from .github/profile-3d-contrib.json. Appears after the first workflow run; see SETUP.md. -->
<p align="center">
